...
def main():
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        ...
        f.write("This document records the baseline accuracy and metrics for the completely untrained (pretrained only) DeBERTa-v3-base model on the `test` split of our RAID-based text dataset. Because the model has not yet been fine-tuned for the binary AI vs. Human text classification task, its performance is expected to be near random chance (~50%). Fine-tuning is explicitly scheduled for Sprint 3.\n\n")
        f.write("## Metrics\n")
