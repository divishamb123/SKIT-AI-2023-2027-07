...
export function FormAlert({
  ...
  const styles = {
    ...
    success: {
      ...
      closeBtn: 'text-emerald-500 hover:bg-emerald-100',
    },
    warning: {
  ...
  return (
    <div
      role="alert"
      className={`flex items-start gap-3 p-3.5 rounded-lg border text-sm transition-all duration-200 ${current.container} ${className}`}
    >
      ...
      {onDismiss && (
        <button
          type="button"
          onClick={onDismiss}
          className={`-mr-1 -mt-1 p-1 rounded-md transition-colors ${current.closeBtn}`}
